...
for line in lines:
	...
	p = r"Train net output #0: edge/loss = [0-9]*\.[0-9]*"
	pattern = re.compile(p)
	matcher = re.search(pattern,line)
	if matcher:
		print matcher.group(0)
		numlist = re.findall('[0-9]\.*[0-9]*',matcher.group(0))
		train_net_output[0].append(float(numlist[-1]))
	p = r"Train net output #1: loss = [0-9]*\.[0-9]*"
	pattern = re.compile(p)
	matcher = re.search(pattern,line)
	if matcher:
		print matcher.group(0)
		numlist = re.findall('[0-9]\.*[0-9]*',matcher.group(0))
		train_net_output[1].append(float(numlist[-1]))
...
plt.figure(1)
min_len = min(len(train_iter),len(train_net_output[0]))
plt.plot(train_iter[:min_len], train_net_output[0][:min_len], 'g')
min_len = min(len(train_iter),len(train_net_output[1]))
plt.plot(train_iter[:min_len], train_net_output[1][:min_len], 'b')
