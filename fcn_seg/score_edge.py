...
def compute_hist(net, save_dir, dataset, layer='score', gt='label'):
    ...
    for idx in dataset:
        ...
        loss_edge += net.blobs['edge/loss'].data.flat[0]
    return hist, loss / len(dataset), loss_edge / len(dataset)
...
def do_seg_tests(net, iter, save_format, dataset, layer='score', gt='label'):
    ...
    hist, loss, loss_edge = compute_hist(net, save_format, dataset, layer, gt)
    # mean loss
    print '>>>', datetime.now(), 'test Iteration', iter, 'loss', loss
    print '>>>', datetime.now(), 'test Iteration', iter, 'loss_edge', loss_edge
    # overall accuracy
